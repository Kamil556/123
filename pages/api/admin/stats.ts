...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const runners   = all!.filter(r => r.role === 'Бегун').length
  const coords    = all!.filter(r => r.role === 'Координатор').length
  const active    = all!.filter(r => r.active === true).length
  const blocked   = all!.filter(r => r.active === false).length
  const withBmi   = all!.filter(r => r.bmi != null)
  ...
  return res.status(200).json({
    ...
    noBmi: all!.length - withBmi.length,
    avgBmi,
    total: all!.length,
  })
}
